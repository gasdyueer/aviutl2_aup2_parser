...
`extensions/` 目录包含基于解析器的领域专用脚本。这些脚本调用 `AUP2Parser` API 完成特定的 AUP2 结构变换，无需修改核心库。
...
```bash
# 将 test2.aup2（8 段平铺在 Layer 0）转换为 9 场景 24 对象的架构
python extensions/convert_flat_to_scene.py test2.aup2 -o output.aup2
```

**`convert_flat_to_scene.py` 变换规则：**
