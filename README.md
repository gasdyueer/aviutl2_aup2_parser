...
- **序列化支持**: 解析结果导出 JSON，从 JSON 反向重建 AUP2
...
| `save_parsed_data(path, encoding='utf-8')` | 解析结果导出为 JSON |
...
```python
from aup2_parser import parse, parse_file, validate_aup2_file, convert_aup2_file
...
**从 JSON/字典重建（非字节精确）**
`reconstruct_from_dict()` 和 `reconstruct_from_json()` 没有原始行记录，走逻辑重建路径：场景按数字排序、使用规范化数值格式。结构正确但可能不字节精确。
